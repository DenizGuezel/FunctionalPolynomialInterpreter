...
tests =
  testGroup "Library Tests"
    ...
    , testCase "überschreibt ein vorhandenes Polynom mit gleichem Namen" $
        savePoly "p1" (P [M 5 1]) [("p1", P [M 3 2])] @?=
          [("p1", P [M 5 1])]

    , testCase "findet ein gespeichertes Polynom" $
        lookupPoly "p2" [("p1", P [M 1 1]), ("p2", P [M 2 2])] @?=
          Just (P [M 2 2])
